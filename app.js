...
function taskTwo() {
  ...
  const randomNum = Math.random() * 999;
  numbers.push(randomNum.toFixed());
...
function taskFive() {
  ...
  answerFive.innerHTML = `<span><b>Ätligt: </b>${eatable}</span> <span><b>Skräp: </b>${trash}</span>`;
  //answerFive.innerHTML = "<b>Ätligt: </b>" + eatable + "<b>Skräp:</b>" + trash;
}
...
function taskSix() {
  ...
  answerSix.innerHTML = `
    ...
    <span><b>Under 30: </b>${underThirty}</span>
    <span><b>Gift: </b>${married}</span>
    <span><b>Ogift: </b>${notMarried}</span>
  `;
}
...
function taskSeven() {
  ...
  const splitText = addMeSomeLineBreaks.split(",");
  answerSeven.innerHTML = splitText.join(",<br>");
}
...
function calculator(valueOne, valueTwo, operator) {
  if (typeof valueOne === "number" && typeof valueTwo === "number") {
   switch (operator) {
     ...
     case "divide":
       if (valueTwo === 0) {
         alert("Använd inte 0 i nämnaren xD")
         break;
        } else {
          alert(valueOne / valueTwo)
          break; 
